...
import com.ranefare.fipe.gateways.domains.*
...
class FipeGatewayImpl(
    ...
    override fun getBrands(vehicleType: VehicleType): List<VehicleBrand> {
        try {
            val fipeVehicleBrands: List<FipeVehicleBrand> = fipeClient.getBrands(
                    FipeVehicleType.valueOf(vehicleType.name).label)
                    .body() ?: throw FipeIntegrationException("Empty result")

            return fipeVehicleBrands.map{
                fipeVehicleBrand -> fipeVehicleBrandToVehicleBrandAssembler.assemble(fipeVehicleBrand, vehicleType)}
        } catch (e: Exception) {
    ...
    override fun getVehicles(vehicleBrand: VehicleBrand): List<Vehicle> {
        try {
            val fipeVehicles: List<FipeVehicle> = fipeClient.getVehicles(
    ...
    override fun getVehicleModels(vehicle: Vehicle): List<VehicleModel> {
        try {
            val fipeVehicleModels: List<FipeVehicleModel> = fipeClient.getVehicleModels(
                    FipeVehicleType.valueOf(vehicle.brand.vehicleType.name).label,
                    vehicle.brand.id,
                    vehicle.id).body() ?: throw FipeIntegrationException("Empty result")

            return fipeVehicleModels.map{
                fipeVehicleModel -> fipeVehicleModelToVehicleModelAssembler.assemble(fipeVehicleModel, vehicle)}
        } catch (e: Exception) {
            throw FipeIntegrationException(message = e.message, cause = e)
        }
    }
    ...
    override fun getVehiclePrice(vehicleModel: VehicleModel): VehicleDetails {
        try {
            val fipeVehicleDetails: FipeVehicleDetails = fipeClient.getPrice(
